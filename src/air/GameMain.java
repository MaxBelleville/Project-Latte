...
public class GameMain {
	...
	public static void main(String[] args) {
		new Window().hideMenu().fullscreen().setBackground(Color.black).icon("bullet.png").init();
		startMenu();
	}
	...
	private static void startMenu() {
		...
		button.addActionListener(e -> {
			if (!Server.serverExists("192.168.50.206", 8000))
				Server.start(8000);
			Client.start();
	...
	public static void keyDown(String key) {
		...
		if (key.equals("Space"))
			player.teleportBack(layers.get(layI), layI);
		if (Handler.isDrawing()) {
			if (key.equals("Up") && layI < layers.size() - 1) {
				if (!layers.get(layI + 1).isHidden())
					layI++;
				else if (layI < layers.size() - 2)
					layI += 2;
			}
	...
	public static void mouseDown(int x, int y, int button) {
		if (Handler.isDrawing()) {
			if (button == 1 && layI == player.getLayer()) {
				Client.send("2500 " + player.getEmpty(0) + " " + Camera.getPos() + " " + new Vector(x, y) + " 8 0");
			}
			if (button == 3)
				player.tryTeleport(layers.get(layI), new Vector(x, y), layI);
		}
	}
	...
	public static void exit(Block border) {
		...
		for (int i = 0; i < bullets.size(); i++)
			bullets.get(i).moveBullet(-disp.getX(), -disp.getY());
	}
	...
	public static void update(Graphics g, double delta) {
		...
		if (Handler.getKeyPressed("Shift"))
			speed = 4 * delta;
		if (Handler.getKeyPressed("W"))
			player.updatePos(player.getPos().getX(), player.getPos().getY() - speed);
		if (Handler.getKeyPressed("A"))
			player.updatePos(player.getPos().getX() - speed, player.getPos().getY());
		if (Handler.getKeyPressed("D"))
			player.updatePos(player.getPos().getX() + speed, player.getPos().getY());
		if (Handler.getKeyPressed("S"))
			player.updatePos(player.getPos().getX(), player.getPos().getY() + speed);
		if (layers.get(layI).get("Wall") != null)
			Handler.onCollide(GameMain::touchWall, player, layers.get(layI).get("Wall"));
		if (layers.get(layI).get("Entites") != null)
			Handler.onCollide(GameMain::touchEntity, player, layers.get(layI).get("Entites"));
		...
		layers.get(layI).draw(g);
			...
			if (Math.abs(vec.getX()) < 1 && Math.abs(vec.getY()) < 1)
				...
				Camera.setPos(0, 0);
			} else if (bullets.get(i).onCollide(layers.get(layI).get("Wall")))
				bullets.remove(i);
			else if (bullets.get(i).onCollide(enemy) && bullets.get(i).getTick() >= 100) {
				System.out.println("Kill");
				bullets.remove(i);
			} else if (bullets.get(i).hasStopped())
				bullets.remove(i);
		}
		g.setColor(Color.black);
		enemy.update(layers,player.getPos(),bullets);
		enemy.fill(g);
		if (player.getLayer() == layI && layers.get(layI).get("Wall") != null)
			player.draw(g, layers.get(layI), mousePos);
		if (!Handler.isDrawing()) {
			sound.pause();
		...
		if (Client.hasData() && !reading) {
			reading = true;
			Handler.onLoopTillComplete(() -> reading = false, GameMain::readServer, p -> (Client.hasData() == p));
		}
	}
