...
public class Layer {
	...
	private Group getGroup(String val) {
		...
		for (String blockStr : split) {
			...
			if (block.getImage().isEmpty())
				block.addRect(x, y, 32, 32);
			if (xySplit[2].equals("Player"))
				block.setColor(Color.green);
			else if (xySplit[2].equals("Enemy"))
				block.setColor(Color.red);
			else if (xySplit[2].equals("StairUp"))
				block.setColor(Color.orange);
			else if (xySplit[2].equals("StairDown"))
	...
	public void setupNav(Vector start, Vector bounds) {
		...
		nav.setupGrid((int) bounds.getX(), (int) bounds.getY(), new Vector(16, 16).add(start), new Vector(32, 32));
